	...
	if ( $jltw_options = get_option( JUIZ_LTW_SETTING_NAME ) ) {
			...
			|| $jltw_options['oauth_token_secret'] == '' ) 
		{
			echo '<div class="error">
				<h3>' . __( "Don't worry!", 'juiz-last-tweet-widget' ) . '</h3>
				<p>' . sprintf( __( 'But you need to %scomplete the %sJuiz Last Tweet Widget%s options page%s if you want to display your latest tweets.' , 'juiz-last-tweet-widget' ), '<a href="' . JUIZ_LTW_SETTINGS_URL . '">', '<strong>', '</strong>','</a>') . '</p>
			</div>';
		}
		if ( ! function_exists( 'curl_init' ) ) {
			echo '<div class="error"><p>';
			echo sprintf( __( 'This plugin use the %scurl_init()%s function, but your PHP configuration doesn\'t allow it. You need to activate this feature to use this plugin. Thank you.', 'juiz-last-tweet-widget' ), '<code>', '</code>' );
			echo '</p></div>';
		}
	}
}
...
if ( function_exists( 'load_plugin_textdomain' ) ) {
	load_plugin_textdomain( 'juiz-last-tweet-widget', false, JUIZ_LTW_DIRNAME . '/languages' );
}
...
class Juiz_Last_Tweet_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			...
			'description'	=> __( 'List you last tweet by displaying content, date, and link to follow you', 'juiz-last-tweet-widget' )
		);

		$control_ops = array();
		
		parent::__construct( 'juiz_last_tweet_widget', __( 'Juiz Last Tweet', 'juiz-last-tweet-widget' ), $widget_ops, $control_ops );
	}
	...
	function Juiz_Last_Tweet_Widget() {
		...
		$this->WP_Widget( 'juiz_last_tweet_widget', __( 'Juiz Last Tweet', 'juiz-last-tweet-widget' ), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$jzoutput .= '
			<p style="border-bottom: 1px solid #DFDFDF;">
				<label for="' . $this->get_field_id('juiz_last_tweet_title') . '"><strong>' . __('Title', 'juiz-last-tweet-widget') . '</strong></label>
			</p>
			<p>
		...
		// Settings
		$jzoutput .= '
			<p style="border-bottom: 1px solid #DFDFDF;"><strong>' . __('Preferences', 'juiz-last-tweet-widget') . '</strong></p>
			
			<p>
				<label>' . __('Username', 'juiz-last-tweet-widget') . '<br />
				<span style="color:#999;">@</span><input id="' . $this->get_field_id('juiz_last_tweet_username') . '" name="' . $this->get_field_name('juiz_last_tweet_username') . '" type="text" value="' . $instance['juiz_last_tweet_username'] . '" /> <abbr title="' . __('No @, just your username', 'juiz-last-tweet-widget') . '">(?)</abbr></label>
			</p>
			<p>
				<label>' . __('Number of tweets to show', 'juiz-last-tweet-widget') . '<br />
				<input style="margin-left: 1em;" id="' . $this->get_field_id('juiz_last_tweet_no_tweets') . '" name="' . $this->get_field_name('juiz_last_tweet_no_tweets') . '" type="text" value="' . $instance['juiz_last_tweet_no_tweets'] . '" /> <abbr title="' . __('Just a number, between 1 and 5 for example', 'juiz-last-tweet-widget') . '">(?)</abbr></label>
			</p>
			<p>
				<label>' . __('Duration of cache', 'juiz-last-tweet-widget') . '<br />
				<input style="margin-left: 1em; text-align:right;" id="' . $this->get_field_id('juiz_last_tweet_cache_duration') . '" name="' . $this->get_field_name('juiz_last_tweet_cache_duration') . '" type="text" size="10" value="' . $instance['juiz_last_tweet_cache_duration'] . '" /> '.__('Seconds', 'juiz-last-tweet-widget').' <abbr title="' . __('A big number save your page speed. Try to use the delay between each tweet you make. (e.g. 1800 s = 30 min)', 'juiz-last-tweet-widget') . '">(?)</abbr></label>
			</p>
			<p>
				<label>' . __('Show your avatar?', 'juiz-last-tweet-widget') . ' 
				<input type="checkbox" name="' . $this->get_field_name('juiz_last_tweet_show_avatar') . '" id="' . $this->get_field_id('juiz_last_tweet_show_avatar') . '"'.$show_avatar_checked.' /> <abbr title="' . __("If it's possible, display your avatar at the top of tweets list", 'juiz-last-tweet-widget') . '">(?)</abbr></label>
			</p>
			<p>
				<label>' . __('Show action links?', 'juiz-last-tweet-widget') . ' 
				<input type="checkbox" name="' . $this->get_field_name('juiz_last_tweet_action_links') . '" id="' . $this->get_field_id('juiz_last_tweet_action_links') . '"'.$action_links_checked.' /> <abbr title="' . __("Display action links like Retweet, Reply and Fav", 'juiz-last-tweet-widget') . '">(?)</abbr></label>
			</p>
			<p>
				<label>' . __('Auto slide one by one?', 'juiz-last-tweet-widget') . ' 
				<input type="checkbox" name="' . $this->get_field_name('juiz_last_tweet_auto_slide') . '" id="' . $this->get_field_id('juiz_last_tweet_auto_slide') . '"'.$auto_slide_checked.' /> <abbr title="' . __("Use JavaScript to activate an little slider showing tweet by tweet", 'juiz-last-tweet-widget') . '">(?)</abbr></label>
			</p>
			<p>
				<label>' . __('Delay between 2 tweets?', 'juiz-last-tweet-widget') . ' 
				<input style="margin-left: 1em; text-align:right;" id="' . $this->get_field_id('juiz_last_tweet_auto_slide_delay') . '" name="' . $this->get_field_name('juiz_last_tweet_auto_slide_delay') . '" type="text" size="10" value="' . $instance['juiz_last_tweet_auto_slide_delay'] . '" /> '.__('Seconds', 'juiz-last-tweet-widget').' <abbr title="' . __("Chose a delay if you use the auto slide feature.", 'juiz-last-tweet-widget') . '">(?)</abbr></label>
			</p>
		';
		
		// Default & Own CSS
		$jzoutput .= '
			<p style="border-bottom: 1px solid #DFDFDF;"><strong>' . __('Manage CSS', 'juiz-last-tweet-widget') . '</strong></p>
			<p>
				<label for="' . $this->get_field_id('juiz_last_tweet_own_css') . '" style="display:inline-block;">' . __('Your own CSS', 'juiz-last-tweet-widget') . ':  <abbr title="' . __('Write your CSS here to replace or overwrite the default CSS', 'juiz-last-tweet-widget') . '">(?)</abbr></label>
	...
	function juiz_last_tweet_output( $args = array(), $position ) {
		...
		if ( ! function_exists( 'jltw_format_since' ) ) {
			function jltw_format_since ( $date ) {
				...
				if ( $diff < 60 ) {
					$the_date .= $diff . ' ';
					$the_date .= ( $diff > 1 ) ?  __( 'Seconds', 'juiz-last-tweet-widget' ) :  __( 'Second', 'juiz-last-tweet-widget' );
				}
				elseif($diff < 3600 ) {
					$the_date .= round( $diff/60 ) . ' ';
					$the_date .= ( round( $diff/60 ) > 1 ) ?  __( 'Minutes', 'juiz-last-tweet-widget' ) :  __( 'Minute', 'juiz-last-tweet-widget' );
				}
				elseif($diff < 86400 ) {
					$the_date .=  round( $diff/3600 ) . ' ';
					$the_date .= ( round( $diff/3600 ) > 1 ) ?  __( 'Hours', 'juiz-last-tweet-widget' ) :  __( 'Hour', 'juiz-last-tweet-widget' );
				}
				else {
					$the_date .=  round( $diff/86400 ) . ' ';
					$the_date .= ( round( $diff/86400 ) > 1) ?  __( 'Days', 'juiz-last-tweet-widget' ) :  __( 'Day', 'juiz-last-tweet-widget' );
				}
		...
		if ( !function_exists('jltw_get_the_user_timeline')) {
				...
				if (!$current_timer OR $current_timer <= time()) {
					...
					if ( is_array( $api_1_1_content ) ) {
						...
						if ($show_avatar && $avatar) { 
							$avatar_attr = array(
								'title'		 => __('Follow', 'juiz-last-tweet-widget') . ' @'.$username.' ' . __('on twitter.', 'juiz-last-tweet-widget'),
						...
						foreach ( $rss_i as $tweet ) {
							...
							//time ago filters
							$the_time_ago = array(
								'before'	=> __('Time ago', 'juiz-last-tweet-widget'),
								'after'		=> '',
								'content'	=> __('See the status', 'juiz-last-tweet-widget')
							);
							...
							if ($show_action_links) {
								...
								$html_action_links ='<span class="juiz_action_links">
									<a title="'.__('Reply', 'juiz-last-tweet-widget').'" href="http://twitter.com/intent/tweet?in_reply_to='.$juiz_tweet_id.'" class="juiz_al_reply" rel="nofollow" target="'.$target4action_links.'">'.__('Reply', 'juiz-last-tweet-widget').'</a> <span class="juiz_ltw_sep">-</span>
									<a title="'.__('Retweet', 'juiz-last-tweet-widget').'" href="http://twitter.com/intent/retweet?tweet_id='.$juiz_tweet_id.'" class="juiz_al_retweet" rel="nofollow" target="'.$target4action_links.'">'.__('Retweet', 'juiz-last-tweet-widget').'</a> <span class="juiz_ltw_sep">-</span>
									<a title="'.__('Favorite', 'juiz-last-tweet-widget').'" href="http://twitter.com/intent/favorite?tweet_id='.$juiz_tweet_id.'" class="juiz_al_fav" rel="nofollow" target="'.$target4action_links.'">'.__('Favorite', 'juiz-last-tweet-widget').'</a> 
								</span>';
							}
					...
					// if any feed is readable
						...
						if( !$is_tweet_cached ) {
							$html_result = '<li><em>' . $html_result . ' '.apply_filters('juiz_ltw_twitter_feed_not_loadable',__('The RSS feed for this twitter account is not loadable for the moment.', 'juiz-last-tweet-widget')).'</em></li>';
			...
			$follow_us = $remove_follow_us ? '' : '<p class="juiz_last_tweet_follow_us">
						<span class="juiz_ltw_follow">' . __('Follow', 'juiz-last-tweet-widget') . '</span>
						<a class="juiz_ltw_username" href="' . esc_url( 'http://twitter.com/' . $the_username ) . '">@' . $the_username . '</a>
						<span class="juiz_ltw_ontwitter">' . __('on twitter.', 'juiz-last-tweet-widget') . '</span>
					</p>';
