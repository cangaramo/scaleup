...
// SQL statement
/*
$sql = "SELECT c.name, c.company_no, c.type, c.incorporation_date, c.employee_count, c.sic_2007, f.turnover, f.turnover_delta, s.description, s.id" . 
" FROM company c".
" INNER JOIN finances f" .
" ON f.company_no = c.company_no " .
" LEFT JOIN sic_codes s " .
" ON s.code = c.sic_2007 "  . $where_query .
" ORDER BY c.company_no ASC";
*/
//Sin left join
$sql = "SELECT c.name, c.company_no, c.type, c.incorporation_date, c.employee_count, c.sic_2007, f.turnover, f.turnover_delta, i.lep_code" . 
" FROM company c".
" INNER JOIN finances f" .
" ON f.company_no = c.company_no" .
" JOIN contact_info i" .
